...
const AddProductVariation = () => {
  ...
  useEffect(() => {
    fetch(`https://seo-server-0f337e2505b4.herokuapp.com/product-variation/${id}`)
      .then((res) => res.json())
  ...
  const handleVariation = (event) => {
    ...
    const url = `https://seo-server-0f337e2505b4.herokuapp.com/add-variation/${variation._id}`;
    fetch(url, {
      method: 'PUT',
      headers: {
