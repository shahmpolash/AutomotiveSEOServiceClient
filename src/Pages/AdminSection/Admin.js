...
const Admin = () => {
    ...
    useEffect(() => {
        fetch(`https://seo-server-0f337e2505b4.herokuapp.com/users`)
            .then((res) => res.json())
            .then((info) => setUsers(info));
    }, []);

    useEffect(() => {
        fetch(`https://seo-server-0f337e2505b4.herokuapp.com/subscribers`)
            .then((res) => res.json())
            .then((info) => setSubscribers(info.reverse())); // Reverse the array initially
    }, []);

    useEffect(() => {
        fetch(`https://seo-server-0f337e2505b4.herokuapp.com/orders`)
            .then((res) => res.json())
