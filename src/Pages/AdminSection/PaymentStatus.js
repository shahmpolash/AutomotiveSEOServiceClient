...
const PaymentStatus = () => {
    ...
    useEffect(() => {
        fetch(`https://seo-server-0f337e2505b4.herokuapp.com/order/${id}`)
            .then((res) => res.json())
    ...
    const handlePaymentStatus = (event) => {
        ...
        const url = `https://seo-server-0f337e2505b4.herokuapp.com/order/${order._id}`;
        fetch(url, {
            method: "PUT",
            headers: {
    ...
    const handleOrderStatus = (event) => {
        ...
        const url = `https://seo-server-0f337e2505b4.herokuapp.com/order-status/${order._id}`;
        fetch(url, {
            method: "PUT",
            headers: {
                "content-type": "application/json",
