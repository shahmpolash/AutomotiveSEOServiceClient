...
const AddProduct = () => {
  ...
  useEffect(() => {
    fetch(`https://seo-server-0f337e2505b4.herokuapp.com/products-variation`)
      .then((res) => res.json())
      .then((info) => setVariations(info));
  }, []);
  useEffect(() => {
    fetch(`https://seo-server-0f337e2505b4.herokuapp.com/categories`)
      .then((res) => res.json())
  ...
  const handleAddProduct = (event) => {
    ...
    const url = `https://seo-server-0f337e2505b4.herokuapp.com/add-product`;
    fetch(url, {
      method: "POST",
      headers: {
